...
include('../dist/includes/dbcon.php');
	...
	$qty = $_POST['qty'];
		...
		$query1=mysqli_query($con,"select * from temp_trans where prod_id='$name' and location_id='$location'")or die(mysqli_error());
		$count=mysqli_num_rows($query1);
		
		$total=$price*$qty;
		
		if ($count>0){
			mysqli_query($con,"update temp_trans set qty=qty+'$qty',price=price+'$total' where prod_id='$name' and location_id='$location'")or die(mysqli_error());
